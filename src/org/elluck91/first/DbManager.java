...
public class DbManager implements IRepo{
	...
	public int Register(User user) {
		try {
			...
			sql.setString(3, user.Role);
			sql.setString(4, user.Email);
	...
	public int UpdatePost(int ID, Post newPost) {
		try {
			...
			PreparedStatement sql = con.prepareStatement("UPDATE Posts SET PostTitle=?,PostContent=?,PostDescription=?,PostImage=?,PublishDate=?,PostAuthor=?,IsVisiable=?,AllowComments=? WHERE ID=?");
